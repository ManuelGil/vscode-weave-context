/**
 * Core file discovery engine for the extension.
 *
 ...
 * Call `clearCache()` after commands, generators, or file watchers change files.
 */
...
const buildIgnoreMatcher = async (options: {
  ...
  if (enableGitignoreDetection) {
    try {
      ...
      const fileContent = await readFileContent(gitignoreUri);
...
const discoverFilesLocally = async (
  ...
  const globStream = FastGlob.stream(normalizedIncludeGlobs, {
    ...
    absolute: true,
  }) as AsyncIterable<string> & { destroy?: () => void };

  try {
    for await (const matchedFilePath of globStream) {
      if (!matchedFilePath) {
        continue;
      }
      ...
      if (totalCollected >= maxResults) {
        ...
        if (typeof globStream.destroy === 'function') {
          try {
            globStream.destroy();
          } catch {
